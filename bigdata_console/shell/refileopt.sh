...
if [ "${transferMode}" == "local2remote" ];then
	...
	result=$?
	message="file transfer operation: ${sourceDir} -> ${DEST_USER}@${Remote}:${destinationDir}/ [Done]"
elif [ "${transferMode}" == "remote2local" ];then
	rsync -avz --progress --delete "${SOURCE_USER}@${Remote}:${sourceDir}" "${destinationDir}/" > /dev/null
	result=$?
	message="file transfer operation: ${SOURCE_USER}@${Remote}:${sourceDir} -> ${destinationDir}/ [Done]"
else
