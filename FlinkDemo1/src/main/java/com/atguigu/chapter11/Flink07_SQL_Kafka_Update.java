...
public class Flink07_SQL_Kafka_Update {
    public static void main(String[] Args) {
        ...
        Table result = tEnv.sqlQuery("SELECT id,sum(vc) as sum_vc FROM sensor group by id");  // 如果有聚合，则结果无法写入到Kafka
...
//                "   ts BIGINT," +
                ...
                "  'format' = 'json'" +
                ")");
        // Table sink 'default_catalog.default_database.abc' doesn't support consuming update changes which is produced by node GroupAggregate(groupBy=[id], select=[id, SUM(vc) AS sum_vc])
