...
public class Dws09_DwsTradeSkuOrderWindow_Cache extends BaseAppV1 {
     ...
     * 补充维度信息
     *
     * 注：此处连续6次查询数据库，比如sku_id，如果数据量很大，会严重影响性能
     * 可以进行缓存优化：第一次从数据库查询，以后从缓存查询
     *    可以存储在：
     *      1. 堆内存(flink状态)
     *          好处：本地内存，读写不需要网络IO，速度极快
     *          坏处：占用flink的计算内存；维度会变化，堆内存中的维度没有办法即使更新
     *      2. 外置内存（redis） 旁路缓存
     *          坏处：每次都需要通过网络
     *          好处：维度变化可以及时更新
     *
     * @param beanStreamWithoutDim
     * @return
     */
