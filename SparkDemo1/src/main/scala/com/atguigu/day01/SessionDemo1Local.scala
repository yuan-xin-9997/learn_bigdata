...
case class UserAnalysis(userId:String,
                        actionTime:String,
                        page:String,
...
object SessionDemo1Local {
  ...
  def main(args: Array[String]): Unit = {
    // 1. 创建SparkContext对象
    val conf = new SparkConf().setMaster("local[*]").setAppName("session demo").set("spark.testing.memory","2147480000")
    ...
    val rdd2: RDD[UserAnalysis] = rdd1.map(
      line => {
        ...
        UserAnalysis(userid, actionTime, page, ts)
      }
    )
    rdd2.collect()
