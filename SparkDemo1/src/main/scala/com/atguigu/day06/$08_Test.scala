...
class $08_Test {
  ...
  def main( ): Unit = {
    ...
    joinDF.show()
      // todo 如果是class类，对比$08_Demo中的object，此处会报错org.apache.spark.SparkException: Task not serializable
      //      而08_Demo不会报错，因为object底层实现了序列化接口
      //      之所以要序列化，是因为要将class和object序列化传递给task
